...
export async function buildApp() {
  ...
  await app.register(cors, {
    ...
    allowedHeaders: ["Content-Type", "Authorization", "X-Requested-With", "X-PWA", "Accept"],
  });
  ...
  const { adminStaffRoutes } = await import("./routes/admin-staff.js");
  await app.register(adminStaffRoutes, { prefix: "/api/v1/admin/staff" });
  ...
  const { hellAiRoutes } = await import("./routes/hell-ai.js");
  await app.register(hellAiRoutes, { prefix: "/api/v1/hell-ai" });
  ...
  const { supportRoutes, adminSupportRoutes } = await import("./routes/support.js");
  ...
  // Промокоды магазина.
  const { promoRoutes, adminPromoRoutes } = await import("./routes/promo.js");
  await app.register(promoRoutes, { prefix: "/api/v1/promo" });
  await app.register(adminPromoRoutes, { prefix: "/api/v1/admin/promo" });

  const { spinRoutes, adminSpinRoutes } = await import("./routes/spin.js");
