...
function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await apiFetch<{ ok: true; mailDelivered: boolean }>(
        "/api/v1/auth/recovery/email/send-link",
        { method: "POST", body: JSON.stringify({ email: email.trim().toLowerCase() }) },
      );
      setSent(true);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Не удалось отправить письмо, попробуйте позже");
    } finally {
      setLoading(false);
    }
  };
        ...
        {sent ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="group relative block w-full overflow-hidden bg-primary py-6 disabled:opacity-60 text-center font-display text-2xl  uppercase font-bold tracking-widest text-black transition-all duration-300 active:scale-[0.97]"
