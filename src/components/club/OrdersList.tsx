// Общий список заказов: фильтры + карточки + empty.
// Используется на /club/orders как самостоятельная страница
...
export function OrdersList({
  ...
  const filtered = useMemo(() => {
    let arr = items;
    if (showFilters) {
      if (filter === "active") arr = arr.filter((o) => ACTIVE.includes(o.status));
      else if (filter === "done") arr = arr.filter((o) => DONE.includes(o.status));
    }
    if (limit) arr = arr.slice(0, limit);
    return arr;
  }, [filter, items, showFilters, limit]);
